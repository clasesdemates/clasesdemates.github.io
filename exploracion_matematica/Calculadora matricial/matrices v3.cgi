...
sub orden_matriz {
	my $r_mat = shift;
	my ($fil, $col) = cuenta_filas_y_columnas ($r_mat);
	my $orden_matriz = min ($fil, $col);

	foreach $orden_matriz(reverse (1...$orden_matriz))
	{

		my @subconjuntos_en_las_filas = 
			subconjuntos_tamanio_fijo(0, $fil-1, $orden_matriz);

		my @subconjuntos_en_las_columnas = 
			subconjuntos_tamanio_fijo(0, $col-1, $orden_matriz);
 
		foreach $i (@subconjuntos_en_las_filas)
		{
			foreach $j( @subconjuntos_en_las_columnas)
			{
				return $orden_matriz if( determinante(menor($r_mat, $i, $j)) );
    			}
    		}
 	}
